"jquery, leaflet, leaflet.markercluster, proj4leaflet, epsg/28992, leafletjs/CoordinatesControl, entities/EM, js/Deferred";
...
function pdok() {
	...
	return new L.TileLayer('http://geodata.nationaalgeoregister.nl/tms/1.0.0/brtachtergrondkaart/{z}/{x}/{y}.png', {
	    minZoom: 0,
	    maxZoom: 28,
	    tms: true,
...
function rd2() {
	...
	return new L.Proj.CRS( 'EPSG:28992','+proj=sterea +lat_0=52.15616055555555 +lon_0=5.38763888888889 +k=0.9999079 +x_0=155000 +y_0=463000 +ellps=bessel +units=m +towgs84=565.2369,50.0087,465.658,-0.406857330322398,0.350732676542563,-1.8703473836068,4.0812 +no_defs',
	    {
			resolutions: [3440.640, 1720.320, 860.160, 430.080, 215.040, 107.520, 53.760, 26.880, 13.440, 6.720, 3.360, 1.680, 0.840, 0.420, 0.210, 0.105, 0.0525, 0.02625],
...
function request_meetpunten(vars, deferred) {
	deferred = deferred || new Deferred();
	
	var markers = vars.markers;
	
	EM.query("../stuv//veldoffice:Meetpunt", "xcoord,ycoord", {
		where: EM.eb.neq("xcoord", null).and(EM.eb.neq("ycoord", null)),
		limit: 25000,
		start: markers.getLayers().length
	}).addCallback(function(res) {
		var arr = [];
		res.tuples.forEach(function(tuple) {
			var pt = {x:tuple[0], y:tuple[1]};
			Proj4js.transform(vars.epsg28992, vars.wgs84, pt);
			arr.push(L.marker([pt.y, pt.x]));
		});
		markers.addLayers(arr);
		if(markers.getLayers().length < res.count) {
			request_meetpunten(vars, deferred);
		} else {
			deferred.callback();
		}
		return res;
    	...
    	setTimeout(function() {
	        ...
	        var vars = this.mixInVars({
			    "map": L.map(scope['map-panel'].getChildNode(0), {
	        		...
	        		crs: rd2(),//L.CRS.proj4js("EPSG:28992"),
	         		center: [52.05, 5.27],
	        		zoom: 3,
	        		preferCanvas: !true
	        	}),
			...
			request_meetpunten(vars).addCallback(function() {
				alert("Done loading meetpunten");
			});
